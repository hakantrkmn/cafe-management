...
interface TakeawayOrderDialogProps {
  ...
  cartTotal: number;
  existingOrders: TakeawayOrderWithRelations[];
  paidOrders: TakeawayOrderWithRelations[];
  onAddToCart: (
...
export function TakeawayOrderDialog({
  ...
  const handleAddToCart = async (
    ...
    console.log("TakeawayOrderDialog - handleAddToCart called");
    ...
    try {
      ...
      console.log("TakeawayOrderDialog - item saved directly");
    } catch (error) {
    ...
    console.log("TakeawayOrderDialog - handleAddToCart completed");
  };

  const handleClose = (open: boolean) => {
    console.log(
      "TakeawayOrderDialog - handleClose called with open:",
      open,
      "isAddingToCart:",
      isAddingToCart
    );
    if (!open && !isAddingToCart) {
      console.log("TakeawayOrderDialog - closing dialog");
      onOpenChange(false);
      setSelectedMenuItem(null);
      setSelectedSize(undefined);
      setExtraDialogOpen(false);
      ...
      console.log(
        "TakeawayOrderDialog - ignoring close request during add to cart"
      );
    }
  };
                ...
                <div className="h-full overflow-y-auto bg-slate-50 p-3 sm:p-6">
                  <div className="order-dialog-container">
                    <OrderSummary
                      ...
                      onRefresh={onRefresh}
                      isSaving={isSaving}
                      selectedTableName="Paket Servis"
                    />
                  </div>
                </div>
              </TabsContent>
